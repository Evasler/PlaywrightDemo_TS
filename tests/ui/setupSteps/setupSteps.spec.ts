...
extendedTest.use({ setupStepsArgsArray: [ setupStepsDataset.setupStepsArgsArray, { scope: 'test' } ] });
extendedTest(testUtils.fullTitle(setupStepsDataset.testDetails.id, setupStepsDataset.testDetails.title, setupStepsDataset.testDetails.suiteTags), async({ openNewTabInNewContext }) => {
