...
function getProjectErrors(projectName: string) {
    const projectErrors = [];
    if (configurations) {
        const projectConfigurations = configurations.filter(configuration => projectName.includes(configuration));
        if (configurations.length > 0) {
            if (projectConfigurations.length === 0)
                projectErrors.push(`Project \"${projectName}\" does not specify the configuration (${configurations.join("|")})`);
            else if (projectConfigurations.length > 1)
                projectErrors.push(`Project \"${projectName}\" specifies multiple configurations (${configurations.join("|")})`);
        }
    }
...
async function getTestErrors(test: TestCase, projectName: string, excelFileIsValid: boolean) {
    ...
    if (configurations && excelFileIsValid && testFileErrors.length === 0) {
        const projectConfigurations = configurations.filter(configuration => projectName.includes(configuration));
...
async function testPointExists(test: TestCase) {
    ...
    try {
        ...
        _projectConfiguration = testUtils.projectConfiguration(test, configurations);
...
const excelReportHelper = {
    ...
    init(excelData: { filepath: string, configurationNames: string[] }) {
        ...
        configurations = excelData.configurationNames;
    },
    ...
    async throwReportingErrors(rootSuite: Suite) {
        ...
        for (const projectSuite of rootSuite.suites) {
            ...
            for (const test of projectSuite.allTests())
                testErrors.push(...await getTestErrors(test, projectName, excelFileErrors.length === 0));
        }
