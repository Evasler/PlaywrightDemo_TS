...
const terminalUtils = {
    ...
    printHeader() {
        console.log(`|${_padText("filename")}|${_padText("title")}|${_padText("level")}|${_padText("message")}|${reset}`);
    },

    printLogLevelMessage(filepath: string, title: string, level: LogLevel, message: string) {
        ...
        console.log(`${textColor}|${_padText(path.basename(filepath))}|${_padText(title)}|${_padText(level)}|${_padText(message)}|${reset}`);
    },

    printColoredText(text: string, color: "red" | "green" | "yellow" | "blue" | "magenta") {
        let textColor: string;
        switch(color) {
            case "blue":
                textColor = fg_blue;
