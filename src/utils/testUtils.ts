...
const testUtils = {
  fullTitle(id: number, title: string, suiteTags?: SuiteTag[]) {
    ...
    if (suiteTags) testTitle += ` | ${suiteTags.join(" ")}`;
    return testTitle;
  },
  ...
  id(test: TestCase) {
    ...
    if (colonIndex < 0)
      throw new Error(
        `Test Title "${test.title}" should follow the format "testId: testTitle (| @tag)?"`,
      );
    const id = test.title.substring(0, colonIndex);
    if (!/\d+/.test(id)) throw new Error(`Test Id "${id}" should be a number`);
    return id;
  },
  ...
  project(test: TestCase) {
    ...
    while (projectSuite.type !== "project") {
      if (!projectSuite.parent)
        throw new Error(
          "Suite parent is undefined while searching for project suite.",
        );
    ...
    if (!project)
      throw new Error(
        `Project suite "${projectSuite.title}" does not have a project defined.`,
      );
    return project;
  },
  ...
  projectConfiguration(test: TestCase, configurationNames: string[]) {
    ...
    const projectConfiguration = configurationNames.filter(
      (configurationName) => projectName.includes(configurationName),
    );
    if (projectConfiguration.length === 0)
      throw new Error(
        `Project "${projectName}" doesn't specify a configuration (${configurationNames.join("|")})`,
      );
  ...
  type(test: TestCase) {
    ...
    const testFilepathSpecifiesApi = /(\\|\/)api(\\|\/)/.test(
      test.location.file,
    );
    if (testFilepathSpecifiesUi && !testFilepathSpecifiesApi) return "UI";
    else if (!testFilepathSpecifiesUi && testFilepathSpecifiesApi) return "API";
  ...
  setReporterStatus({
    ...
    fileUtils.writeFile(
      ...
      JSON.stringify(reporterStatus, null, 4),
    );
  },
