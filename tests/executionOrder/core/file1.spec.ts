...
test.describe("describe1", () => {
    ...
    test("[2] describeTest", async ({ manualTestFixture, manualWorkerFixture }, testInfo) => {  // eslint-disable-line @typescript-eslint/no-unused-vars
        await test.step("step1", () => { terminalUtils.printLogLevelMessage(import.meta.filename, testInfo.title, "test", "step1") });
        await test.step("step2", async () => {
            terminalUtils.printLogLevelMessage(import.meta.filename, testInfo.title, "test", "step2_1");
            await test.step("nestedStep", () => { terminalUtils.printLogLevelMessage(import.meta.filename, testInfo.title, "test", "nestedStep") });  // eslint-disable-line playwright/no-nested-step
            terminalUtils.printLogLevelMessage(import.meta.filename, testInfo.title, "test", "step2_2");
        });
    });
