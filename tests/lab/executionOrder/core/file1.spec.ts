...
test.describe("describe1", () => {
    ...
    test("[2] describeTest", async ({ manualTestFixture, manualWorkerFixture }, testInfo) => {
        ...
        await test.step("step2", async () => {
            TerminalUtils.printColoredMessage(__filename, testInfo.title, "test", "step2_1");
            await test.step("nestedStep", async () => TerminalUtils.printColoredMessage(__filename, testInfo.title, "test", "nestedStep"));
            TerminalUtils.printColoredMessage(__filename, testInfo.title, "test", "step2_2");
        });
    });
