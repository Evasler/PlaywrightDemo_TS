...
export default class PlansSteps {
  ...
  async getRequestError(planId: number, suiteId: number, testCaseId: number) {
    try {
      const response = await this._plansRequests.getPointsFilteredByTestCaseId(
        planId,
        suiteId,
        testCaseId,
      );
  ...
  async getTestPointId(
    ...
    try {
      ...
      const responseJson = (await response.json()) as {
        value: { id: number; configuration: { id: string; name: string } }[];
      };
      const testPoint = responseJson.value.filter(
        (testPoint) => testPoint.configuration.name === configurationName,
      );
      if (testPoint.length === 1) return testPoint[0].id;
      return undefined;
    } catch {
      return undefined;
    }
