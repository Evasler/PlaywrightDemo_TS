...
class AdminPanelSteps extends BaseSteps {
    ...
    deleteRoom(roomInfo: { roomName: string, type: string, accessible: string, price: string, roomDetails: string }) {
        this.addStep(`Deleting Room [${roomInfo.roomName}, ${roomInfo.type}, ${roomInfo.accessible}, ${roomInfo.price}, ${roomInfo.roomDetails}]`, async() => {
            console.log(`Deleting Room [${roomInfo.roomName}, ${roomInfo.type}, ${roomInfo.accessible}, ${roomInfo.price}, ${roomInfo.roomDetails}]`);
    ...
    verifyRoomVisibility(roomInfo: { roomName: string, type: string, accessible: string, price: string, roomDetails: string }, shouldBeVisible: boolean) {
        this.addStep(`Verifying Room [${roomInfo.roomName}, ${roomInfo.type}, ${roomInfo.accessible}, ${roomInfo.price}, ${roomInfo.roomDetails}] is ${shouldBeVisible ? "" : "not "}visible`, async() => {
            console.log(`Verifying Room [${roomInfo.roomName}, ${roomInfo.type}, ${roomInfo.accessible}, ${roomInfo.price}, ${roomInfo.roomDetails}] is ${shouldBeVisible ? "" : "not "}visible`);
            if (shouldBeVisible)
