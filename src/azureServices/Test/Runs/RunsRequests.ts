...
export default class RunsRequests {
  ...
  constructor(
    private readonly _authorizedContext: APIRequestContext,
    baseUrl: string,
  ) {
  ...
  postRun(payload: RunDetails) {
    return this._authorizedContext.post(this._runsEndpoints.runs(), {
      data: payload,
    });
  }
  ...
  patchRun(runId: number, payload: RunDetails) {
    return this._authorizedContext.patch(this._runsEndpoints.run(runId), {
      data: payload,
    });
  }
  ...
  patchResult(runId: number, payload: ResultDetails) {
    return this._authorizedContext.patch(this._runsEndpoints.results(runId), {
      data: [payload],
    });
  }
}
